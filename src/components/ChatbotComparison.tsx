...
const ChatbotComparison = () => {
  ...
  const faqMessages: Message[] = [{
    role: "user",
    content: "Ik zoek rode Nike sneakers in maat 42, heb je die op voorraad?",
    delay: 500
  }, {
    role: "assistant",
    content: "Hoi! Ik ben een chatbot. Ik kan je helpen met algemene vragen.",
    delay: 1500
  }, {
    role: "user",
    content: "Ja maar heb je rode Nike sneakers maat 42?",
    delay: 1000
  }, {
    role: "assistant",
    content: "Sorry, ik begrijp je vraag niet. Probeer één van deze opties: 1) Openingstijden 2) Retourbeleid 3) Verzendkosten",
    delay: 1500
  }, {
    role: "user",
    content: "😤 (verlaat website)",
    delay: 800
  }];
  const glimpsMessages: Message[] = [{
    ...
    role: "assistant",
    content: "Hey! 👋 Ik zie dat je rode Nike sneakers in maat 42 zoekt. Ik heb 23 opties voor je!",
    delay: 1000
  }, {
    role: "assistant",
    content: "Op basis van je eerdere Nike voorkeur raad ik deze aan:\n\n🔥 Nike Air Max 1 Rood - €129,95 (⭐4.8, op voorraad)\n🔥 Nike Dunk Low University Red - €139,95 (⭐4.9, 2 stuks over)",
    delay: 2000
  }, {
    role: "user",
    content: "Perfect! Wat is de levertijd?",
    delay: 1000
  }, {
    role: "assistant",
    content: "Voor 22:00 besteld = morgen in huis! 📦 Gratis verzending boven €50. Wil je ze in je winkelmandje? 😊",
    delay: 1200
  }];
  useEffect(() => {
  ...
  return <section className="py-20 px-6 bg-background">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-center gap-4 mb-12 flex-wrap">
          <Button variant={selectedBot === "none" ? "default" : "outline"} size="lg" onClick={() => handleBotSelect("none")} className={`gap-2 ${selectedBot === "none" ? "bg-foreground text-background" : ""}`}>
            <Users className="w-5 h-5" />
            Geen Bot
          </Button>
          <Button variant={selectedBot === "faq" ? "default" : "outline"} size="lg" onClick={() => handleBotSelect("faq")} className={`gap-2 ${selectedBot === "faq" ? "bg-[#FF8C42] hover:bg-[#FF8C42]/90 text-white" : "border-[#FF8C42]/30"}`}>
            <Store className="w-5 h-5" />
            Simpele FAQ Bot
            <AlertTriangle className="w-4 h-4 text-[#FF8C42]" />
          </Button>
          <Button variant={selectedBot === "glimps" ? "default" : "outline"} size="lg" onClick={() => handleBotSelect("glimps")} className={`gap-2 ${selectedBot === "glimps" ? "bg-primary hover:bg-primary/90 text-white" : ""}`}>
        ...
        <div className="max-w-6xl mx-auto">
          {(selectedBot === null || selectedBot === "none") && <div className="grid md:grid-cols-3 gap-6">
              {/* Geen Bot Card */}
          ...
          {selectedBot === "faq" && <div className="bg-card border-2 border-[#FF8C42] rounded-3xl overflow-hidden">
              ...
              <div className="h-[400px] overflow-y-auto p-6 space-y-4 bg-muted/20">
                {messages.slice(0, currentMessageIndex).map((message, index) => <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                    <div className={`max-w-[80%] p-4 rounded-2xl ${message.role === 'user' ? 'bg-foreground text-background' : 'bg-card border border-[#FF8C42]/30'}`}>
          ...
          {selectedBot === "glimps" && <div className="bg-card border-2 border-primary rounded-3xl overflow-hidden">
              ...
              <div className="h-[400px] overflow-y-auto p-6 space-y-4 bg-muted/20">
                {messages.slice(0, currentMessageIndex).map((message, index) => <div key={index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                    <div className={`max-w-[80%] p-4 rounded-2xl ${message.role === 'user' ? 'bg-foreground text-background' : 'bg-card border border-primary/30'}`}>
                      <p className="text-sm leading-relaxed whitespace-pre-line">{message.content}</p>
                      {index === messages.length - 1 && message.role === "assistant" && <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
